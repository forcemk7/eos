...
-- T1: resume snapshots tied to job listings (tailoring)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- T6: off-platform apply logging (outbound clicks, decisions) + append-only events for funnel analytics
DO $$
BEGIN
...
-- T7: optional link from cover letter chat to tracked job_listing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- T5: Strong match loop (search + fit iterations)
